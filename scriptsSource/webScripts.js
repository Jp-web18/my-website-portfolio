...
// active state for each navigation buttons
window.onload = function() {
	const currentUrl = window.location.href;
	const currentPage = currentUrl.split('/').pop().split('.').shift();

	if (currentPage === "main") {
		homePageActive();
	} else if (currentPage === "blogPageFile") {
		blogPageActive();
	} else if (currentPage === "flexPageFile") {
		flexPageActive();
	}
};
